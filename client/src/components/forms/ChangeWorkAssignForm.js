...
export default function ChangeWorkAssignForm({ onClose, onSubmit, addemployeeid }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log("entered into selected comapnies try block");
      const response = await axios.post(
        process.env.REACT_APP_API_URL + 'api/changeAssignCompanies', // Backend endpoint
        {
      ...
      console.log("entered into selected comapnies");
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        toast.error("Failed to Access changed. Try again.");
      }
    } catch (error) {
      console.error("Error in Access changed:", error);
