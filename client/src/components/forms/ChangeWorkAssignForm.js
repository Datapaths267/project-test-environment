...
export default function ChangeWorkAssignForm({ onClose, onSubmit, addemployeeid }) {
    ...
    useEffect(() => {
      axios
        .get(process.env.REACT_APP_API_URL + "countryAll")
    ...
    const fetchCompaniesByCountry = async (country) => {
      ...
      try {
        const response = await axios.get(
          process.env.REACT_APP_API_URL + `companiesbycounty/${country}`
        );
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        const response = await axios.post(
          process.env.REACT_APP_API_URL + "changeAssignCompanies", // Backend endpoint
          {
